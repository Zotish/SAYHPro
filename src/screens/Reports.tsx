  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 pb-24 lg:pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-nv-900">{isBn ? "আর্থিক ও ব্যবসায়িক রিপোর্ট" : "Financial & Business Reports"}</h1>
          <p className="text-nv-500 text-xs sm:text-sm mt-0.5">{isBn ? "লাভ-ক্ষতি ও পারফরম্যান্স বিশ্লেষণ" : "Profit & Loss, Sales breakdown, and expense audit"}</p>
        </div>
        ...
        {[
          ...
          <button
            ...
            className={`px-4 py-2 text-xs sm:text-sm font-bold rounded-xl transition-fast whitespace-nowrap
              ${activeTab === tab.id ? "bg-em-700 text-white shadow-xs" : "text-nv-600 hover:bg-nv-100"}`}
          >
      ...
      {activeTab === "pl" && (
        <div className="bg-white rounded-3xl shadow-sm border border-nv-200 p-6 space-y-6">
          <div className="border-b border-nv-100 pb-4 flex items-center justify-between">
            <div>
              <h3 className="font-display font-bold text-nv-900 text-lg">{isBn ? "লাভ ও ক্ষতি বিবরণী (P&L Statement)" : "Profit & Loss Statement"}</h3>
              <p className="text-xs text-nv-500">Live generated from verified transactions</p>
            </div>
            <span className="text-xs px-3 py-1 bg-em-50 text-em-800 font-bold rounded-full border border-em-200">
              Audited Ready
            </span>
          </div>
          ...
          <div className="space-y-4 max-w-2xl text-xs sm:text-sm">
            ...
            <div className="space-y-2 pt-2">
              <div className="font-bold text-nv-900 border-b border-nv-100 pb-1">
              ...
              {Object.keys(expenseByCategory).length === 0 && (
                <div className="text-nv-400 pl-4 text-xs">No operating expenses recorded yet</div>
              )}
      ...
      {activeTab === "sales" && (
        <div className="bg-white rounded-3xl shadow-sm border border-nv-200 p-6 space-y-4">
          <h3 className="font-display font-bold text-nv-900 text-lg">{isBn ? "সকল বিক্রয় তালিকা" : "Completed Sales Records"}</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs sm:text-sm">
              <thead>
                <tr className="bg-nv-50 border-b border-nv-200">
                  <th className="px-4 py-3 font-bold text-nv-600">Invoice No</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Customer</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Payment</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Subtotal</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Discount</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Total</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Date & Time</th>
                </tr>
              </thead>
      ...
      {activeTab === "expenses" && (
        <div className="bg-white rounded-3xl shadow-sm border border-nv-200 p-6 space-y-4">
          <h3 className="font-display font-bold text-nv-900 text-lg">{isBn ? "খরচের সম্পূর্ণ তালিকা" : "Expense Audit Trail"}</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs sm:text-sm">
              <thead>
                <tr className="bg-nv-50 border-b border-nv-200">
                  <th className="px-4 py-3 font-bold text-nv-600">Category</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Amount</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Paid From</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Date</th>
                  <th className="px-4 py-3 font-bold text-nv-600">Note</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-nv-100">
                {expenses.map(e => (
      ...
      {activeTab === "dues" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-nv-200 p-5 space-y-3">
            <h3 className="font-display font-bold text-nv-900 text-base">{isBn ? "গ্রাহক বাকি (Receivables)" : "Customer Receivables"}</h3>
          ...
          {/* Supplier Payables */}
          <div className="bg-white rounded-3xl shadow-sm border border-nv-200 p-5 space-y-3">
            <h3 className="font-display font-bold text-nv-900 text-base">{isBn ? "সাপ্লায়ার দেনা (Payables)" : "Supplier Payables"}</h3>
            <div className="divide-y divide-nv-100">
